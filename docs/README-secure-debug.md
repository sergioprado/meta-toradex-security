...
| iMX8M Plus   | SJC     | Not supported (SoC erratum ERR052318, see *iMX8M Plus* below) |
...
The class is currently restricted by an explicit machine allow-list (`verdin-imx8mm`), held in `TDX_SECURE_DEBUG_SUPPORTED_MACHINES`. Each backend appends the machines it has been validated on, and the generic class checks the list. Building for any other machine with `TDX_SECURE_DEBUG_ENABLE=1` fails at sanity-check time — including SoC families that have no backend yet, so the feature never silently does nothing.

### iMX8M Plus

Secure JTAG is **not usable** on this part. NXP states in AN4686 that "the Secure Debug
mode is not functional on the i.MX 8M Plus" and points to device erratum **ERR052318**.
The challenge/response mechanism is the affected part, so
`TDX_SECURE_DEBUG_MODE="authenticated"` must never be offered on iMX8MP even if the rest
of the backend is otherwise ready.

The blocking modes are a different matter: nothing in the erratum affects `JTAG_SMODE=11`
(No Debug) or `SJC_DISABLE`, so those remain the way to lock JTAG down on this SoC.

The machine is simply absent from `TDX_SECURE_DEBUG_SUPPORTED_MACHINES` today, so any
build for it fails at sanity-check time regardless of mode. Should support ever be added,
two iMX8MP specifics matter: authenticated mode has to be rejected explicitly, and the
part's response key does not fit the iMX8MM fuse map — AN4686 notes it needs two
additional `SJC_RESP` banks, so it requires its own `*-sjc-template.fuse`.
...
TDX_SECURE_DEBUG_KEY_FILE = "/path/to/keys/secure-debug/key.txt"
```
...
| `TDX_SECURE_DEBUG_KEY_FILE` | Path to the file containing the 56-bit response key, used when `TDX_SECURE_DEBUG_MODE="authenticated"`. The file must exist; the build fails otherwise. | `${TOPDIR}/keys/secure-debug/key.txt` |
...
The build generates one set of fuse commands, so **every device programmed from a
given build carries the same response key**. That is a deliberate trade-off, not an
oversight: the commands are produced at build time, before any particular device is
known, and a per-device key cannot be derived from a device UID that does not exist
yet. The consequence is worth stating plainly — if that one key leaks, every device
fused with it can be opened over JTAG.

AN4686 §5 describes three ways to manage response keys, of which only the first fits
a build-time flow:

| Policy | Fits this class | Notes |
| :----- | :-------------- | :---- |
| Identical key on every device | yes | Simplest, and the weakest. One leak exposes the whole fleet. |
| Database of per-device keys | no | Strongest. Needs a provisioning step that reads each device's UID and burns a matching key, plus a server holding the challenge/response pairs. |
| Key derived algorithmically from the UID | no | No database to keep, but the scheme becomes worthless once the algorithm leaks or is reverse-engineered. |

If your threat model cannot accept a fleet-wide key, do not use this class to program
`SJC_RESP`: provision the response key per device in your manufacturing flow instead,
and use the class only to select the JTAG mode. Treat the key file exactly like the HAB
signing keys — keep it out of the repository, out of build logs, and under the same
access control.

### After authentication

Authentication is per-session, not permanent. Any reset returns the SJC to its locked
state, so the debug probe must repeat the challenge/response exchange after every
reset. This is worth knowing when scripting a debug session: a script that resets the
target must re-authenticate before it can attach again.

### Closing the device

If `TDX_SECURE_DEBUG_ENABLE=1`, at the end of the build the secure-debug fuse commands are appended to the existing `fuse-cmds.txt` in the image deploy directory, between the SRK-hash writes and the `SEC_CONFIG[1]` close command. The fuses they program are also recorded in `imx-config.fuse`, which is a map of what gets burned rather than an ordered script. The ordering matters: response-key fuses must be written before the device is closed.
...
- NXP Application Note AN4686, *Secure Debug in i.MX 6/7/8M Family of Applications Processors*, Rev. 4.0, 5 February 2025 (SJC operating principles, the challenge/response flow, and response-key management).
